## Mechanical engineering
### General CAD
+ [colah's ImplicitCAD thread on HN](https://news.ycombinator.com/item?id=9248174)
+ [Wikipedia: solvespace](https://en.wikipedia.org/wiki/SolveSpace)
+ [SolveSpace website](http://solvespace.com/index.pl)
+ [Wikipedia: CGAL](https://en.wikipedia.org/wiki/CGAL)
+ [Wikipedia: OpenSCAD](https://en.wikipedia.org/wiki/OpenSCAD)
+ [RapCAD](https://github.com/GilesBathgate/RapCAD)
+ [Matt Keeter: Antimony](http://www.mattkeeter.com/projects/antimony/3/)


### Finite element analysis
+ [Wikipedia: FEM in structural mechanics](https://en.wikipedia.org/wiki/Finite_element_method_in_structural_mechanics)
  + [Wikipedia: Gaussian quadrature](https://en.wikipedia.org/wiki/Gaussian_quadrature)
  + [Wikipedia: Modal analysis using FEM](https://en.wikipedia.org/wiki/Modal_analysis_using_FEM)
+ [Wikipedia: Direct stiffness method](https://en.wikipedia.org/wiki/Direct_stiffness_method)
  + [Wikipedia: Flexibility method](https://en.wikipedia.org/wiki/Flexibility_method)
  + [Wikipedia: von Mises yield criterion](https://en.wikipedia.org/wiki/Von_Mises_yield_criterion)
  + [Wikipedia: Cauchy stress tensor](https://en.wikipedia.org/wiki/Cauchy_stress_tensor)
  + [Wikipedia: Linear elasticity](https://en.wikipedia.org/wiki/Linear_elasticity)
  + [Wikipedia: Stress functions](https://en.wikipedia.org/wiki/Stress_functions)
+ [Wikipedia: Cholesky decomposition](https://en.wikipedia.org/wiki/Cholesky_decomposition)
  + [Wikipedia: Definiteness of a matrix](https://en.wikipedia.org/wiki/Definiteness_of_a_matrix)
  + [Wikipedia: Hermitian matrix](https://en.wikipedia.org/wiki/Hermitian_matrix)
+ [Wikiversity: nonlinear finite element analysis](https://en.wikiversity.org/wiki/Nonlinear_finite_elements)
  + [Wikipedia: minimum total potential energy principle](https://en.wikipedia.org/wiki/Minimum_total_potential_energy_principle)
  + [Wikipedia: virtual work](https://en.wikipedia.org/wiki/Virtual_work)
  + [Wikipedia: Lagrangian mechanics](https://en.wikipedia.org/wiki/Lagrangian_mechanics)
  + [Wikiversity: introduction to elasticity/tensors](https://en.wikiversity.org/wiki/Introduction_to_Elasticity/Tensors)
  + [Wikiversity: calculus of variations](https://en.wikiversity.org/wiki/Nonlinear_finite_elements/Calculus_of_variations)
+ [GMSH source code: elasticity solver](https://gitlab.onelab.info/gmsh/gmsh/blob/master/Solver/elasticitySolver.h)
  + [Wikipedia: Dirichlet boundary condition](https://en.wikipedia.org/wiki/Dirichlet_boundary_condition)
  + [Wikipedia: Neumann boundary condition](https://en.wikipedia.org/wiki/Neumann_boundary_condition)
+ [Wikipedia: structural dynamics](https://en.wikipedia.org/wiki/Structural_dynamics)
+ [SO: Why is the stress tensor linear and symmetric?](https://physics.stackexchange.com/questions/62963/why-is-the-nonrelativistic-stress-tensor-linear-and-symmetric)


### Solid modeling
+ [Wikipedia/solid modeling](https://en.wikipedia.org/wiki/Solid_modeling)
+ [StackOverflow: kdtrees vs octrees](https://cstheory.stackexchange.com/questions/8470/why-would-one-ever-use-an-octree-over-a-kd-tree)


### Dealing with isosurfaces
+ [Wikipedia: isosurface](https://en.wikipedia.org/wiki/Isosurface)
+ [Boris the Brave: dual contouring](https://www.boristhebrave.com/2018/04/15/dual-contouring-tutorial/)
+ [Matt Keeter: 2D contouring](https://www.mattkeeter.com/projects/contours/)
+ [Wikipedia: dual graph](https://en.wikipedia.org/wiki/Dual_graph)
+ [Wikipedia: implicit surface](https://en.wikipedia.org/wiki/Implicit_surface)
